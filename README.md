# 👋 Hey, I'm Gwydeon

💻 **Developer focused on building tools for games, communities, and the web.**

I enjoy working at the intersection of gaming and development, creating projects that improve user experience, automate workflows, and power online communities.

---

## 🚀 What I do

🌐 **Web Development**  
- Custom web applications  
- Backend specialization with **Laravel**  
- Modern frontend technologies and architectures  
- Performance, scalability, and clean code  

🤖 **Discord Bots**  
- Custom bots for communities and projects  
- Automation, integrations, and utilities  
- Strong focus on gaming and community management  

🧪 **Hytale Plugins / Mods**  
- Custom mechanics and systems  
- Exploring modding possibilities from the ground up  

🧩 **World of Warcraft Addons**  
- Gameplay enhancements and quality-of-life tools  
- UI improvements, helpers, and automation  

---

## 🛠️ Tech Stack

- **Languages:** JavaScript, PHP, Lua  
- **Backend:** Laravel, APIs, authentication, data modeling  
- **Frontend:** Modern JS, component-based architectures  
- **Gaming:** WoW Addons, Modding  
- **Bots:** Discord.js  
- **Other:** Git, automation, project architecture  

---

## 🔗 Accounts & Links

- 💬 **Discord:** `gwydeonn`  
- 🎮 **Battle.net:** `GwydeoN#2140`  
- 📺 **Twitch:** https://www.twitch.tv/gwydeonn  
- 🐦 **X (Twitter):** https://x.com/Gwydeonn  
- 🕹️ **Steam:** https://steamcommunity.com/id/gwydeonn/  

---

✨ *Code. Play. Improve.*
